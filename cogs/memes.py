...
class Memes:
	...
	async def kg_sim(self, ctx):
		...
		def choose_chars(all_chars_list):
			...
			p_svr = random.gauss(0.25, 0.2) # Percentage of regular cast that can survive, must be > 0
			n_svr = 0
			if (round(p_svr * len(reg_chars)) <= 0):
				n_svr = 1

			else:
				n_svr = round(p_svr * len(reg_chars))
		...
		# Array to dump all lines in 
		story = []

		n_chars = len(characters)
		if(cont_outer and n_chars < 8):
			story.append(f"You need at least 8 characters to play a game!")
			cont_outer = False
		...
		if (cont_outer):
			...
			story.append(raw_setting.replace("{n_chars}", f"**{n_chars}**"))
			...
			# story.append(f"Regular: {reg_chars}\nMM: {mm_chars}\nTraitor: {t_chars}\nNo. Survivors: {n_svr}")

		# Prologue
		if (cont_outer):
			# await asyncio.sleep(3)	
			story.append("**__~Prologue Start~__**")
			...
			while (len(unmentioned_chars) > 0):
				...
				if (p > p_rd):
					...
					# story.append(unmentioned_chars)
				else: # death event
			...
			# await asyncio.sleep(3)
			story.append(prologue_str)
			# await asyncio.sleep(3)
			story.append("**__~Prologue End~__**")
			story.append(get_headcount(reg_chars, mm_chars, t_chars, reg_dead, mm_dead, t_dead))
		...
		if (cont_outer):
			...
			while (len(reg_chars) > n_svr):
				...
				story.append(f"**__~Ch. {ch_idx} Start~__**")
				...
				if (len(reg_chars) <= n_svr):
					...
					if (p < p_td and len(t_chars) > 0): # Traitor death
						...
						# await asyncio.sleep(8)
						story.append(f"***DING DONG BING BONG***\n{bda_parsed}")
						...
						story.append(f"**__~Trial {ch_idx} Start~__**")
						story.append(trial_str)
						...
						if (p <= p_success):
							...
							story.append(f"**{ci_er}** delivers a CI and reveals the truth!")
							story.append(case_parsed)
							...
							story.append("No one was able to discover the truth in time!")
							story.append(case_parsed)
						...
						# await asyncio.sleep(8)
						story.append(f"***DING DONG BING BONG***\n{bda_parsed}")
						# story.append(f"**Case**: {case_parsed}")
				...
				# End of chapter
				story.append(f"**__~Ch. {ch_idx} End~__**")
				story.append(get_headcount(reg_chars, mm_chars, t_chars, reg_dead, mm_dead, t_dead))

				ch_idx += 1
				
				# Check if there is at least one regular member left
				if (len(reg_chars) < 1):
					mm_won = True
					break
			...
			if (mm_won): # Lost trial
				story.append("Sadly for everyone, the truth slipped away and the mastermind(s) won...")
				...
				try:
					...
					story.append(f"In a final showdown, **{revealer}** discovers the truth behind the killing game...")
					mm_dead.extend(mm_chars)
					mm_chars = []
				except IndexError: # No regular characters left (last regular character was a victim)
					if (len(t_chars) != 0 and len(mm_chars) != 0): # Traitor(s) turn on the MM
						revealer = random.choice(t_chars)
						story.append(f"In a final showdown, **{revealer}** reveals the truth behind this farce of a trial, turning on the ringleader(s)...")
						mm_dead.extend(mm_chars)
					elif (len(t_chars) != 0): # NO mm, and some traitors
						revealer = random.choice(t_chars)
						story.append(f"In a final showdown, **{revealer}** reveals the truth behind this farce of a trial...")
					else: # No traitors, MM
						await ctx.send("Congratulations! You've hit a rare bug! Fix in progress.")
			...
			story.append(f"**Mastermind(s)**: {mm_str}\n\n**Traitor(s)**: {traitor_str}\n\n**Dead**: {dead_str}\n\n**Survivor(s)**: {survivors_str}")

		# Dump story array to an html file
		if (cont_outer):
			fname = f"{ctx.guild.id}-{ctx.channel.id}.txt"

			with open(fname, 'w+') as fh:
			    for line in story:
			        fh.write(line)

			await ctx.send("Finished! [file here]")
